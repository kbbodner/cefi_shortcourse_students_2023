...
It's also common to parameterize this model using $S_{max} = 1/\beta$, which is the number of spawners that produces the maximum number of recruits (the peak of the Ricker curve,) since it is more intuitive to put priors around. $$
...
SimData <- Sim_Ricker_SR_Data(leng=50, age=4, Sig_Ricker = 0.15, true_a = rnorm(1, 5, 2), true_b=1/100000, hr_min = 0.2, hr_max = 0.8, lnorm_corr = FALSE)
...
SimDF <- SimData$DF_Out %>% mutate(logRPS = log(R/S))
lm_Fit <- lm(logRPS ~ S, data=SimDF)
alpha_est <- exp( as.numeric(coef(lm_Fit)[1]) )
# look at alpha_est
alpha_est
# compare to
SimData$true_a
...
beta_est <- -1 * as.numeric( coef(lm_Fit)[2] )
# look at beta_est
beta_est
#compare to
SimData$true_b
...
Reference points are used in fisheries management to glean population health and status, and also as trigger points for fisheries management actions (ie change fishing regulations). Two often-used reference points for salmon, are \$S\_{MSY}\$ and \$S\_{gen}\$. To understand these two reference points, let's start with the idea of maximum sustainable yield. Maximum sustainable yield is highest catch that can be taken from a population indefinitely, at equilibrium population conditions. This is considered a "sweet spot" of productivity, and usually occurs at a stock size just before the peak of the stock-recruit curve, which occurs at \$S\_{Max}\$, or the number of spawners that leads to maximum recruitment (often considered "habitat capacity", since beyond this point we start to see density dependence occur). In most fisheries we talk about \$B\_{MSY}\$ or \$F\_{MSY}\$ which are biomass and fishing mortality at MSY, but for salmon, since we are usually managing spawners, we use \$S\_{MSY}\$. \$S\_{MSY}\$ is generally used as an upper benchmark, or a goal, for a fish stock. On the opposite end, an often-used lower benchmark or limit reference point (LRP) for salmon is \$S\_{gen}\$, the number of spawners required to reach \$S\_{MSY}\$ in one generation, in the absence of fishing.Neither of these values have straight-forward solutions when using a Ricker curve. Up until a few years ago \$S\_{MSY}\$ was generally solved using the following approximation, which was introduced by Hilborn (1985).
...
# Can also just give this function the time series
BMs_out_ts <- GetRickerBMs(SRDat=SimData$DF_Out, a=NA, B=NA)
...
# read in real data
DataIn <- read.csv("DataIn/FraserSockeyeData2022.csv")
# let's just choose the first population 
ESData <- DataIn %>% filter(Pop_Name == "Early Stuart")

# This is data available to forecast 2022, so what would have been available in the fall of 2021
# the last complete brood is 2016, who's age-5s would have returned in 2021
# the last spawner estimate is 2020, because there is often a delay in these estimates

# let's use my function GetRickerBMs

RickerEsts <- GetRickerBMs(SRDat=ESData, a=NA, B=NA, Hilborn_approx=F)

# Let's see how these look on top of the population
ggplot(data=RickerEsts$dat) + geom_point(aes(x=S, y=R)) + 
  geom_line(aes(x=S, y=R_Est), col = "blue") + 
  geom_vline(Data = RickerEsts$SMSY, aes(xintercept=RickerEsts$SMSY), color="green", size=1.5 ) +
  geom_vline(Data = RickerEsts$Sgen, aes(xintercept=RickerEsts$Sgen), color="red", size=1.5) +
  theme_bw()

# Fit doesn't look great, being skewed by a few very high returns, which are skewing benchmarks high

# what is most recent average spawner numbers?
# as a convention, we tend to use a "generational average" but you can also compare this to most recent one year of spawners

mean(ESData$S[ESData$yr %in% c(2017:2020)])

# Way below Sgen. Not looking great for this population. This population was heavily impacted by a landslide in 2019, so has seen historically low spawner numbers since.

# You'd have to go back to 2001 to see a spawner count above the lower benchmark

# Sadly this is the case for many Fraser stocks, that are facing warmer ocean and freshwater temperatures, habitat alteration, and continued exploitation.
...
# Let's look at our outputs from above
RickerEsts$dat

# from this we have estimates of total recruits from the 2016 and 2017 brood years, everything we need to forecast 2021 returns

# Let's start by estimating average age proportions

Prop_Age4 <- mean(ESData$rec4/ESData$R, na.rm=T)

# This stock is mostly age-4's!

Age_4_2021_Est <- Prop_Age4 * RickerEsts$dat$R_Est[RickerEsts$dat$yr == 2017] 
Age_5_2021_Est <- (1-Prop_Age4) * RickerEsts$dat$R_Est[RickerEsts$dat$yr == 2016]

Total_2021_Est <- Age_4_2021_Est + Age_5_2021_Est

True_2021_Return <- ESData$rec4[ESData$yr == 2017] + ESData$rec5[ESData$yr == 2016]

# Looks like we actually would have under-forecast in 2021, 
...
#| include: false
library(R2jags)
Forecast2021 <- RunModRetro(Dat=ESData, Pred_Year=2021)
Forecast2021$Preds_Out
# Looks pretty similar to our crude estimate
