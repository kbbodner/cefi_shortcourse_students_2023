...
4. From the Git tab, click the box next to the file you just created. This is equivalent to `git add`
5. Click Commit, enter a log message, and click Commit. This is equivalent to `git commit`
6. To push the change up to Github click on the green up arrow. This is equivalent to `git pus`
...
4. Open a new file, enter the code below, and then save the file as `2_calibrate_forecast.R`
```{r}
##' Calibrate aquatic forecast model
calibrate_forecast <- function(target){
  fit <- list() 
