...
covid_data <- read_csv(here("./CaseStudy_Covid19/covid-bc/data/validation_data.csv"))
...
bc_data1 <- read_csv(here("./CaseStudy_Covid19/covid-bc/data/bc_fitting_data.csv"))

# Load Total BC cases for whole province for first 60 recorded time-points
bc_data2 <- read_csv(here("./CaseStudy_Covid19/covid-bc/data/bc_validation_data.csv"))
...
```{r read stan model}
code_extended <- readLines(here("./CaseStudy_Covid19/covid-bc/stan_models/extended_seir_model.stan"))
...
model_extended <- stan_model(here("./CaseStudy_Covid19/covid-bc/stan_models/extended_seir_model.stan"))
```
...
ggsave(here("./CaseStudy_Covid19/covid-bc/team-4/figures/Infected.mask.jpeg")
       , fit_extended %>%
...
############### Exposed individuals##################
ggsave(here("./CaseStudy_Covid19/covid-bc/team-4/figures/Exposed.mask.jpeg"), fit_extended %>%
  spread_draws(y_hat[t, i]) %>%
  left_join(state_dictionary, by = "i") %>%
  left_join(time_dictionary, by = "t") %>%
  filter(state == "E") %>%
...
ggsave(here("./CaseStudy_Covid19/covid-bc/team-4/figures/Recovered.mask.jpeg"), fit_extended %>%
  spread_draws(y_hat[t, i]) %>%
  left_join(state_dictionary, by = "i") %>%
  left_join(time_dictionary, by = "t") %>%
  filter(state == "R") %>%
...
ggsave(here("./CaseStudy_Covid19/covid-bc/team-4/figures/Susceptible.mask.jpeg"), fit_extended %>%
  spread_draws(y_hat[t, i]) %>%
  left_join(state_dictionary, by = "i") %>%
  left_join(time_dictionary, by = "t") %>%
  filter(state == "S") %>%
...
ggsave(here("./CaseStudy_Covid19/covid-bc/team-4/figures/fitted.mask.jpeg"), fit_extended %>%
