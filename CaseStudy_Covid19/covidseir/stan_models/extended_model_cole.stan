...
functions {
  real[] seir_rhs(real t, // time (actual time; not an increment starting at 1)
    ...
    real logR0_baseline = theta[1];
    real logpan_impact = theta[2];


    real sigma = x_r[1];
    real gamma = x_r[2];
    real pop_size = x_r[3];
    real time_masking = x_r[4];
    real logmask_impact = x_r[5];
    real time_start_of_pandemic = x_r[6];

    real R0 = exp(logR0_baseline - (t > time_masking) * logmask_impact -
    (t > time_start_of_pandemic) * logpan_impact);
...
transformed data {
  ...
  x_r[3] = pop_size;
  x_r[4] = time_masking;
  //x_r[5] = logmask_impact;
  //x_r[6] = time_start_of_pandemic;
}
