...
# Load Total BC cases for whole province for first 60 recorded time-points
bc_validation_data <- read_csv("/Users/johnmensah/Documents/Ecological\ Forecasting_Workshop/Team\ 4/CaseStudy_Covid19/covidseir/data/bc_validation_data.csv")

new.Reported_Date <- unique(bc_validation_data$Reported_Date)
...
bc_validation_data <- rbind(bc_data, bc_validation_data)
```
...
fitting_valid_points <- bc_validation_data$time_points
fitting_valid_cases <- bc_validation_data$n
forecast_valid_points <- seq(max(fitting_valid_points) + 1,
                            max(fitting_valid_points) + 31)

forecast_valid_dates <- seq(max(bc_validation_data$Reported_Date) + 1,
                      max(bc_validation_data$Reported_Date) + 31, by = "1 day")
```
...
##########################
stan_valid_data <- list(
  T = length(fitting_valid_points), # number of data points
  y = fitting_valid_cases, # observed infection cases
  ts = fitting_valid_points, # Time points
  forecast_T = length(forecast_valid_points), # number of forecast points
  forecast_ts = forecast_valid_points, # Forecast time points
  R0_prior = c(log(2.5), 0.2), # log Mean and std for R0
  i0_prior = c(log(8), 1.0), # log Mean and std for i0
  gamma = 1 / 7, # recovery rate
  sigma = 1 / 5, # incubation rate
...
fit_valid <- sampling(model,
  data = stan_valid_data,
  iter = 1000,
  seed = 42, # fix seed to recreate results
  chains = 2,
  control = list(adapt_delta = 0.9)
...
####################################
summary(fit_valid, c("R0", "i0", "sample_frac"))
```

# Examining the model output for the original data
...
ggsave("susceptible_old.jpeg", fit %>%
  spread_draws(y_hat[t, i]) %>%
  left_join(state_dictionary, by = "i") %>%
  left_join(time_dictionary, by = "t") %>%
  filter(state == "S") %>%
...
time_dictionary1 <- bc_validation_data %>%
  select(Reported_Date) %>%
  mutate(t = 1:n())

########## Infected Individuals###########
ggsave("Infected_new.jpeg", fit_valid %>%
  spread_draws(y_hat[t, i]) %>%
  left_join(state_dictionary, by = "i") %>%
  left_join(time_dictionary1, by = "t") %>%
  filter(state == "I") %>%
  ggplot(aes(x = Reported_Date)) +
  stat_lineribbon(aes(y = y_hat), .width = c(.99, .95, .8, .5)) +
  scale_fill_brewer() + ylab("Infected Individuals") + theme_bw() )
...
############### Exposed individuals##################
ggsave("Exposed_new.jpeg", fit_valid %>%
  spread_draws(y_hat[t, i]) %>%
  left_join(state_dictionary, by = "i") %>%
  left_join(time_dictionary1, by = "t") %>%
  filter(state == "E") %>%
...
############### Recovered Individuals##################
ggsave("Recovered_new.jpeg", fit_valid %>%
  spread_draws(y_hat[t, i]) %>%
  left_join(state_dictionary, by = "i") %>%
  left_join(time_dictionary1, by = "t") %>%
  filter(state == "R") %>%
...
############### Susceptible Individuals##################
ggsave("susceptible_new.jpeg", fit_valid %>%
  spread_draws(y_hat[t, i]) %>%
  left_join(state_dictionary, by = "i") %>%
  left_join(time_dictionary1, by = "t") %>%
  filter(state == "S") %>%
...
fit_valid %>%
  ...
  stat_lineribbon(aes(y = cases), .width = c(.99, .95, .8, .5), color = "#08519C") +
  scale_fill_brewer() +
  geom_point(data = bc_validation_data, aes(x = Reported_Date, y = n))
```
...
ggsave("forecast.old_data.jpeg", fit %>%
...
time_dictionary_fitting2 <- bc_validation_data %>%
  select(Reported_Date) %>%
  mutate(
    t = 1:n(),
    .variable = "cases"
  )
time_dictionary_forecasting2 <- tibble(Reported_Date = forecast_dates) %>%
  mutate(
    t = 1:n(),
    .variable = "forecasted_cases"
  )

time_dictionary2 <- time_dictionary_fitting2 %>%
  bind_rows(time_dictionary_forecasting2)

ggsave("forecast.new_data.jpeg",fit_valid %>%
  tidybayes::gather_draws(cases[t], forecasted_cases[t]) %>%
  left_join(time_dictionary2, by = c("t", ".variable")) %>%
  ggplot(aes(x = Reported_Date)) +
  stat_lineribbon(aes(y = .value, group = .variable),
    color = "#08519C",
    .width = c(.8, .5)
  ...
  scale_fill_brewer() +
  geom_point(data = bc_validation_data, aes(x = Reported_Date, y = n)) + ylab("Forecasted cases")) + 

```

#. Extracting forecated cases in the old data and the corresponding true fiited cases in the new data

```{r}
old.forecast <- fit %>%
  tidybayes::gather_draws(cases[t], forecasted_cases[t]) %>%
  left_join(time_dictionary, by = c("t", ".variable")) %>%
filter(Reported_Date %in% new.Reported_Date) %>%
  group_by(t) %>%
  summarise(mean.cases = mean(.value)) %>%
  ungroup()



new.fitted <- fit_valid %>%
  spread_draws(cases[t]) %>%
  left_join(time_dictionary1, by = "t")%>%
filter(Reported_Date %in% new.Reported_Date) %>%
  group_by(t) %>%
...
data.comp <- data.frame(cbind("new.data"=new.fitted$mean.cases, "old.forecasted.data"=old.forecast$mean.cases), "Reported.Date" = new.Reported_Date) 

ggsave("Forecast_vs_Actual.jpeg", ggplot(data = data.comp, aes(x=new.data, y=old.forecasted.data)) + geom_point() + geom_smooth(method='lm', se = T) + ylab("Forecasted cases from old data") + xlab("Actual cases from new data") + theme_bw() + ggtitle("Our model forecasted more cases than what was actually recorded"))
```
...
data.com1 <- fit_valid %>%
  tidybayes::gather_draws(cases[t]) %>%
  left_join(time_dictionary2, by = c("t", ".variable")) %>%
  left_join(bc_validation_data, by = "Reported_Date") %>%
  mutate(residuals = n - .value)
...
```{r}
ggsave("residuals_plot.jpeg", data.com1  %>%
  group_by(t) %>% 
...
ggsave("resid.plot_old.jpeg", data.com  %>%
  group_by(t) %>% 
  summarise(mean_residuals = mean(residuals)) %>% 
  ggplot(aes(x = t, y=mean_residuals)) + 
  geom_point() + 
