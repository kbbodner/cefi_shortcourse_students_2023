...
covid_data <- read_csv("/Users/johnmensah/Documents/Ecological\ Forecasting_Workshop/Team\ 4/CaseStudy_Covid19/covidseir/data/validation_data.csv")
...
bc_data1 <- read_csv("/Users/johnmensah/Documents/Ecological\ Forecasting_Workshop/Team\ 4/CaseStudy_Covid19/covidseir/data/bc_fitting_data.csv")

# Load Total BC cases for whole province for first 60 recorded time-points
bc_data2 <- read_csv("/Users/johnmensah/Documents/Ecological\ Forecasting_Workshop/Team\ 4/CaseStudy_Covid19/covidseir/data/bc_validation_data.csv")

bc_data.new <- rbind(bc_data1, bc_data2)
...
bc_data.new %>%
  ...
  geom_line() +
  theme_bw() +
  annotate('rect', xmin= ymd("2020-03-29"), xmax= ymd("2020-04-12"), ymin = 0, ymax = 95, fill='red', alpha=0.5) + ylab("Cases") + xlab("Reported cases")
...
fitting_time_points <- bc_data.new$time_points
fitting_cases <- bc_data.new$n
forecast_time_points <- seq(max(fitting_time_points) + 1,
                            max(fitting_time_points) + 31)

forecast_dates <- seq(max(bc_data.new$Reported_Date) + 1,
                      max(bc_data.new$Reported_Date) + 31, by = "1 day")
```
...
```{r read stan model}
code_extended <- readLines("/Users/johnmensah/Documents/Ecological\ Forecasting_Workshop/Team\ 4/CaseStudy_Covid19/covidseir/stan_models/extended_seir_model.stan")

cat(code_extended, sep = "\n")
```
...
model_extended <- stan_model("/Users/johnmensah/Documents/Ecological\ Forecasting_Workshop/Team\ 4/CaseStudy_Covid19/covidseir/stan_models/extended_seir_model.stan")
```
...
stan_data_extended <- list(
  ...
  t0 = -1,
  logmask_impact = log(0.5),
  time_masking = 52,
  time_start_of_pandemic = 15
)
```
...
```{r}
fit_extended <- sampling(model_extended,
...
```{r}
summary(fit_extended, c("logR0_baseline", "logpan_impact", "i0", "sample_frac"))
```
...
time_dictionary <- bc_data.new %>%
  select(Reported_Date) %>%
  mutate(t = 1:n())

########## Infected Individuals###########
ggsave("Infected.mask.jpeg", fit_extended %>%
...
############### Exposed individuals##################
ggsave("Exposed.mask.jpeg", fit_extended %>%
  spread_draws(y_hat[t, i]) %>%
  left_join(state_dictionary, by = "i") %>%
  left_join(time_dictionary, by = "t") %>%
  filter(state == "E") %>%
...
ggsave("Recovered.mask.jpeg", fit_extended %>%
  spread_draws(y_hat[t, i]) %>%
  left_join(state_dictionary, by = "i") %>%
  left_join(time_dictionary, by = "t") %>%
  filter(state == "R") %>%
...
ggsave("Susceptible.mask.jpeg", fit_extended %>%
  spread_draws(y_hat[t, i]) %>%
  left_join(state_dictionary, by = "i") %>%
  left_join(time_dictionary, by = "t") %>%
  filter(state == "S") %>%
...
fit_extended %>%
  ...
  scale_fill_brewer() +
  geom_point(data = bc_data.new, aes(x = Reported_Date, y = n)) + ylab("Predicted cases") + xlab("Reported Date") + theme_bw() + ggtitle("Forecast for Implementing Face Mask")
```
...
data.com1 <- fit_extended %>%
  tidybayes::gather_draws(cases[t]) %>%
  left_join(time_dictionary, by = c("t", ".variable")) %>%
  left_join(bc_data.new, by = "Reported_Date") %>%
...
```{r}
y_data <- bc_data$n
shinystan::launch_shinystan(fit_extended)
```
